...
final class YPEditorView: UIView {
...
extension YPEditorView {
    
    class func xibView() -> YPEditorView? {
        let bundle = Bundle(for: YPEditorView.self)
        let nib = UINib(nibName: "YPEditorView",
                        bundle: bundle)
        let xibView = nib.instantiate(withOwner: self, options: nil)[0] as? YPEditorView
        return xibView
    }
