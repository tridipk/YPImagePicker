...
extension YPVideoEditorLibraryVC {
    ...
    func startMultipleSelection(at indexPath: IndexPath) {
        ...
        let mediaItem = getYPEditorMediaItems()[index]
        changeAsset(item: mediaItem)
    ...
    func deselect(indexPath: IndexPath) {
        ...
        let mediaItems = getYPEditorMediaItems()
        let mediaItem = mediaItems[index]
        if let positionIndex = selection.firstIndex(where: { $0.assetIdentifier == mediaItem.localIdentifier }) {
            selection.remove(at: positionIndex)

            // Refresh the numbers
            var selectedIndexPaths = [IndexPath]()
            for(index,asset) in mediaItems.enumerated(){
            ...
            if let previouslySelectedIndexPath = selectedIndexPaths.last {
                ...
                let index = previouslySelectedIndexPath.item
                let item = getYPEditorMediaItems()[index]
                changeAsset(item: item)
            }
    ...
    func addToSelection(indexPath: IndexPath) {
        let index = indexPath.item
        let asset = getYPEditorMediaItems()[index]
        selection.append(
            YPLibrarySelection(
                index: indexPath.row,
    ...
    func isInSelectionPool(indexPath: IndexPath) -> Bool {
        let mediaItem = getYPEditorMediaItems()[indexPath.row]
        return selection.contains(where: { $0.assetIdentifier == mediaItem.localIdentifier })
    }
...
extension YPVideoEditorLibraryVC: UICollectionViewDataSource {
    public func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return getYPEditorMediaItems().count
    }
}

extension YPVideoEditorLibraryVC: UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView,
        ...
        let localIdentifier = mediaItem.localIdentifier!
        ...
        DispatchQueue.global(qos: .background).async {
            self.mediaManager.imageManager?.fetchPreviewFor(video: url, targetSize: self.v.cellSize(), callback: { (image) in
                DispatchQueue.main.async {
                    if cell.representedAssetIdentifier == localIdentifier && image != nil {
        ...
        cell.durationLabel.text = YPHelper.formattedStrigFrom(CMTimeGetSeconds(asset.duration))
        ...
        if let index = selection.firstIndex(where: { $0.assetIdentifier == localIdentifier }) {
...
extension YPVideoEditorLibraryVC: UICollectionViewDelegateFlowLayout {
    public func collectionView(_ collectionView: UICollectionView,
