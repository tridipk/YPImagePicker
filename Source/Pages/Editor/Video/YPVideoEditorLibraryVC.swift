...
public class YPVideoEditorLibraryVC: UIViewController, YPPermissionCheckable {
    
    internal weak var delegate: YPLibraryViewDelegate?
    internal var v: YPEditorView!
    ...
    internal let panGestureHelper = PanGestureHelperVideoEditor()
    internal var videoItems = [YPEditorMedia]()
    ...
    public required init(items: [YPMediaItem]?) {
        ...
        if let items = YPConfig.library.editorVideoItems{
            videoItems = items
        }
    }
    ...
    public func getYPEditorMediaItems() -> [YPEditorMedia]{
        return videoItems
    }
    ...
    public override func loadView() {
        v = YPEditorView.xibView()//YPLibraryView.xibView()
        view = v
    }
    ...
    private func showMultipleSelection() {
        ...
        if multipleSelectionEnabled {
            if selection.isEmpty {
                let asset =  videoItems[currentlySelectedIndex]
                selection = [
    ...
    func refreshMediaRequest() {
        if(videoItems.count > 0){
            changeAsset(item: videoItems[0])
            v.collectionView.reloadData()
            v.collectionView.selectItem(at: IndexPath(row: 0, section: 0), animated: false, scrollPosition: UICollectionView.ScrollPosition())
            if !multipleSelectionEnabled {
    ...
    func changeAsset(item: YPEditorMedia){
        latestImageTapped = item.localIdentifier!
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            self.v.assetZoomableView.setVideo(item.url!, mediaManager: self.mediaManager, storedCropPosition: self.fetchStoredCrop(), completion: {
                completion(false)
    ...
    private func fitsVideoLengthLimits(item: YPEditorMedia) -> Bool {
        if let url = item.url{
            let asset = AVAsset(url: url)
            let tooLong = floor(CMTimeGetSeconds(asset.duration)) > YPConfig.video.libraryTimeLimit
            let tooShort = floor(CMTimeGetSeconds(asset.duration)) < YPConfig.video.minimumTimeLimit
            if tooLong || tooShort {
                DispatchQueue.main.async {
                    let alert = tooLong ? YPAlert.videoTooLongAlert(self.view) : YPAlert.videoTooShortAlert(self.view)
                    self.present(alert, animated: true, completion: nil)
    ...
    private func checkVideoLengthAndCrop(for item: YPEditorMedia,
                                         withCropRect: CGRect? = nil,
                                         callback: @escaping (_ videoURL: URL?) -> Void) {
        if fitsVideoLengthLimits(item: item) == true {
            delegate?.libraryViewDidTapNext()
            let normalizedCropRect = withCropRect ?? DispatchQueue.main.sync { v.currentCropRect() }
            if let videoURL = item.url, let ts = targetSize(url: videoURL, cropRect: normalizedCropRect), let size = getAVAssetSize(url: videoURL){
                let xCrop: CGFloat = normalizedCropRect.origin.x * size.width
                let yCrop: CGFloat = normalizedCropRect.origin.y * size.height
                let resultCropRect = CGRect(x: xCrop, y: yCrop, width: ts.width, height: ts.height)
                mediaManager.fetchVideoURLAndCrop(for: videoURL, cropRect: resultCropRect, callback: callback)
            }
        }
    }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if self.multipleSelectionEnabled && self.selection.count > 1 {
                ...
                for item in self.videoItems {
                    if self.fitsVideoLengthLimits(item: item) == false {
                        return
                    }
                }
                ...
                for item in self.videoItems {
                    asyncGroup.enter()
                    let selection = self.selection[n]
                    self.checkVideoLengthAndCrop(for: item, withCropRect: selection.cropRect) { videoURL in
                        if let videoURL = videoURL {
                            let videoItem = YPMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL), videoURL: videoURL)
                ...
                let item = self.videoItems[0]
                self.checkVideoLengthAndCrop(for: item, callback: { videoURL in
                    DispatchQueue.main.async {
                        if let videoURL = videoURL {
                            self.delegate?.libraryViewFinishedLoading()
                            let video = YPMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL), videoURL: videoURL)
                            videoCallback(video)
                        } else {
                            print("YPLibraryVC -> selectedMedia -> Problems with fetching videoURL.")
                        }
    ...
    private func targetSize(url:URL, cropRect: CGRect) -> CGSize? {
        guard let track = AVAsset(url: url).tracks(withMediaType: AVMediaType.video).first else { return nil }
        let size = track.naturalSize.applying(track.preferredTransform)
        var width = (size.width * cropRect.width).rounded(.toNearestOrEven)
        var height = (size.height * cropRect.height).rounded(.toNearestOrEven)
    ...
    private func getAVAssetSize(url:URL) -> CGSize?{
        guard let track = AVAsset(url: url).tracks(withMediaType: AVMediaType.video).first else { return nil }
        let size = track.naturalSize.applying(track.preferredTransform)
        return size
    }
